...
## CSS修正・調整内容（2026年3月25日）

### 電話ボタン
- スマホ表示をPCと同様の横並びレイアウト（電話アイコン・ラベル ｜ 番号・サブテキスト）に統一
- ボタン文言を変更：「緊急時にもご対応させて頂きます」→「緊急時にも対応します！」

### 確かな技術セクション（PC）
- コンテンツ幅を縮小し、左右に余白が出るよう調整

### 全部見せます！換気ダクト清掃セクション（スマホ）
- タイトル・動画・人物写真エリアに左右50pxのパディングを追加
- ボタンはパディング対象外とし、`margin: 0 -50px` + `width: calc(100% + 100px)` でフル幅を維持

### 人物写真のSafari不具合修正（スマホ）
- **症状**：一部iPhoneで人物写真の下に大きな余白が表示される
- **原因**：`height: auto` のコンテナ内で `img { height: 100% }` を指定していたため、Safariの描画エンジン（WebKit）のバージョンにより挙動が異なっていた
- **修正**：コンテナに `height: 210px` を明示し、`img` を `width: 100%; height: auto;` に変更
- ※ PC側は親コンテナに `clamp()` で明示的な高さが設定されているため問題なし

### 説明文の幅調整（スマホ）
- 「見えないダクト内部の汚れは〜」の説明文に `padding: 0 30px` を追加し、STEP1〜3と同じ左右余白（合計50px）に統一
...
### 目的
SWELL子テーマ導入時、LP専用のCSS・JSを特定ページのみに限定し、他ページへの干渉を防ぐ。
※ ボタンが縦積みになる問題はキャッシュが原因であり、functions.phpとは無関係だった。
